...
void StructuralGraph::SGraph::MakeMeshReal(TSharedPtr<Mesh> mesh)
{
		...
		if (from_n->Profile && to_n->Profile)
		{
			...
			for (int i = 0; i < to_n->Profile->NumVerts; i++)
			{
				...
				Util::AddPolyToMeshReversed(mesh, poly,
					...
					to_n->Profile->IsDrivable(i) || from_n->Profile->IsDrivable(i) ? 0 : 1);
...
void StructuralGraph::SGraph::MakeMeshSkeleton(TSharedPtr<Mesh> mesh)
{
	for (const auto& e : Edges)
	{
		auto from_n = e->FromNode.Pin();
		...
		{
			{
				...
				Util::AddPolyToMesh(mesh, verts, PGCEdgeType::Rounded, 0);
			}
			...
			C2CFacePair(mesh,
				...
				PGCEdgeType::Sharp, PGCEdgeType::Rounded, 0);
